...
contract StopLoss {
    ...
    // Working as Excpected | Conditional needed 
    function withdraw(uint _amt, address _token) external returns(bool) {
      ...
      /*
      Add in conditional to remove stop/limit Orders if they have yet to be executed.
  ...
  /*  
      ...
      function upkeepStop() external returns(bool) {
        for (uint i =0; i < stopOrders.length; i++) {
            if (stopOrders[i].dip_amount >= getLatestPrice(stopOrders[i].asset_deposited)) {
                //IERC20 token = IERC20(stopOrders[i].asset_deposited);
                //token.approve(dexRouter, stopOrders[i].total_asset_value);
                uint256 amtOut = getAmountOutMin(stopOrders[i].asset_desired, stopOrders[i].asset_deposited, stopOrders[i].total_asset_value);
    ...
    */
    ...
    function updateBalance(address Token_owner, address _assetDesried, uint256 _amtOutOfSwap ) external {
        balances[Token_owner] = Balance(Token_owner, _assetDesried, _amtOutOfSwap);
    }
